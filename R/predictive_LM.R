...
#' @param dca_thresholds Numeric vector specifying the threshold probabilities used for decision curve analysis (DCA). The default is {seq(0.05, 0.50, by = 0.01)}
#'
...
#'   \item{dca_results: Decision curve analysis results, including threshold probabilities and net benefit for the prediction model, treat-all strategy, and treat-none strategy}
#' }
...
#' data(renal)
#' fit<-LMf(renal ,tw=list(sl=seq(0,10, by=1),w=5),cov=list(fixed=c("age", "weight", "sex"),vary=c("GFR","proteinuria")),id="id",rtime="yearse",time="time",status="status",inter=TRUE)
#' a1<-VS_LM(fit,cov=list(fixed=c("age", "weight", "sex"),vary=c("GFR","proteinuria")))
#' LM_pre<-predictive_LM(a1,method="K-fold",n=1,seed=1,K=5)
predictive_LM <- function(object, method, n, K, seed, r, n_group = 10, dca_thresholds = seq(0.05, 0.50, by = 0.01)) {
  ...
  if (method == "K-fold") {
      ...
      tryCatch({
        ...
        for (j in 1:K) {
          ...
          for (k in seq_along(sl)) {
            ...
            # Store out-of-fold predictions for DCA
            dca_pred_tmp <- data.frame(
              id = data[test_idx1, ][[id]],
              pred_surv = all_pred1,
              time_point = sl[k],
              original_time = data[test_idx1, ]$original_time,
              original_status = data[test_idx1, ]$original_status
            )
        ...
        # ----------------------------------
        # DCA using pooled OOF predictions
        # ----------------------------------
        for (k in seq_along(sl)) {

          dca_data_k <- all_dca_pred[
            all_dca_pred$time_point == sl[k],
          ]

          if (nrow(dca_data_k) == 0) next

          dca_tmp <- cal_dca(
            pred_surv = dca_data_k$pred_surv,
            data = dca_data_k,
            s = sl[k],
            w = w,
          ...
          if (!is.null(dca_tmp)) {
            dca_tmp$run <- i
            dca_cv[[length(dca_cv) + 1]] <- dca_tmp
          }
        }
    ...
    for (i in 1:(5 * n)) {
      tryCatch({
        ...
        for (k in seq_along(sl)) {
          ...
          if (!is.null(dca_boot) && !is.null(dca_orig)) {

            # 只对预测模型本身做optimism correction
            boot_model <- dca_boot[
              dca_boot$variable == "risk",
              c("threshold", "net_benefit")
            ]

            orig_model <- dca_orig[
              dca_orig$variable == "risk",
              c("threshold", "net_benefit")
            ]
            ...
            tmp <- merge(
              ...
              orig_model,
              by = "threshold",
              suffixes = c("_boot", "_orig")
            )

            tmp$optimism <-
              tmp$net_benefit_boot -
              tmp$net_benefit_orig

            tmp$run <- i
            tmp$time_point <- sl[k]

            dca_opt[[length(dca_opt) + 1]] <- tmp
          }
    ...
    # Combine apparent DCA
    dca_app_all <- do.call(
      rbind,
      dca_app[!sapply(dca_app, is.null)]
    )

    # Combine bootstrap optimism
    dca_opt_all <- do.call(
      rbind,
      dca_opt
    )
    mean_dca_opt <- aggregate(
      optimism ~ time_point + threshold,
      data = dca_opt_all,
      FUN = function(x) mean(x, na.rm = TRUE)
    )
    dca_model_app <- dca_app_all[
      dca_app_all$variable == "risk",
    ]
    dca_model_corr <- merge(
      dca_model_app,
      mean_dca_opt,
      by = c("time_point", "threshold"),
      all.x = TRUE
    )
    dca_model_corr$net_benefit <-
      dca_model_corr$net_benefit -
      dca_model_corr$optimism
    dca_reference <- dca_app_all[
      dca_app_all$variable != "risk",
    ]
    dca_model_corr$optimism <- NULL
    common_cols <- intersect(
      names(dca_model_corr),
      names(dca_reference)
    )
    dca_results <- rbind(
      dca_model_corr[, common_cols],
      dca_reference[, common_cols]
    )
    ...
    for (i in 1:(5 * n)) {
      tryCatch({
        ...
        for (k in seq_along(sl)) {
          ...
          if (!is.null(dca_tmp)) {
            dca_tmp$run <- i
            dca_split[[length(dca_split) + 1]] <- dca_tmp
          }
          all_surv <- cbind(
    ...
    # Summarize DCA across repeated splits
    if (length(dca_split) > 0) {

      dca_split_all <- do.call(rbind, dca_split)

      dca_results <- aggregate(
        net_benefit ~ time_point + variable + label + threshold,
        data = dca_split_all,
        FUN = function(x) mean(x, na.rm = TRUE)
      )
