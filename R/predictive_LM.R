...
predictive_LM<-function(object,method,n,K,seed,r){
  ...
  if (method=="CV"){
    cindex_sm<-data.frame(array(NA,dim=c((5*n),length(sl))))
    BS_sm<-data.frame(array(NA,dim=c((5*n),length(sl))))
    AUC_sm<-data.frame(array(NA,dim=c((5*n),length(sl))))
    nrun<-0
    for(i in 1:(5*n)){
      tryCatch({
        set.seed(seed+10*i )
  ...
  if (method=="boot"){
    ...
    cindex_opt<-BS_opt<-AUC_opt<-data.frame(array(NA,dim=c((5*n),length(sl))))
    nrun<-0
    for (i in 1:(5*n)){
      tryCatch({
        set.seed(seed+10*i)
