#' Landmarking Model Fitting Function
#'
...
#' @param tw  A list containing two elements: `sl` (landmark time points) and `w` (prediction window).
#' @param cov A list containing two elements: `fixed` (fixed covariates) and `vary` (time-varying covariates).
#' @param id Character string specifying the subject ID variable
#' @param rtime Observation time column name
#' @param time Character string specifying the event time variable.
#' @param status Character string specifying the event status variable.
#' @param pre_cov Character vector of pre-specified covariates with dynamic effects(optional)
#' @param inter Logical indicating whether to include interactions with landmark times (default=TRUE)
#' @param func_covars The functional form of the dynamic effect of covariates. When inter is TRUE, the available functional forms include linear functions (linear), quadratic functions (quadratic), exponential functions (exp), and logarithmic functions (log), with the default being linear and quadratic functions.
#' @param func_lms The functional forms for the dynamic effects of baseline hazard rates. When inter is TRUE, the available functional forms include linear functions (linear), quadratic functions (quadratic), exponential functions (exp), and logarithmic functions (log), with the default being linear and quadratic functions.
#'
#' @return  An object of class "LMf" containing:
#'  \describe{
...
#' print(fit)
LMf<-function(data,tw=list(sl=sl,w=w),cov=list(fixed=fixed,vary=vary),id=id,rtime=rtime,time=time,status=status,pre_cov=NULL,
              inter=TRUE,func_covars=c("linear","quadratic"),func_lms=c("linear","quadratic")){
  if (!require("dynpred")) {
  ...
  if (is.null(cov$vary)){
    LMdata<-NULL
    fixed1<-c(id,cov$fixed, "original_time", "original_status")
    for (j in seq(along=tw$sl)){
      LM<-dynpred::cutLM(data,outcome=list(time=time,status=status),
                         LM=tw$sl[j],horizon=tw$sl[j]+tw$w,covs=list(fixed=fixed1,varying=cov$vary))
    ...
    for (j in seq(along=tw$sl)){
      LM<-dynpred::cutLM(data,outcome=list(time=time,status=status),
                         LM=tw$sl[j],horizon=tw$sl[j]+tw$w,covs=list(fixed=c(cov$fixed,"original_time", "original_status"),varying=cov$vary),format="long",id=id,rtime=rtime,right=F)
      #LM$pseudo=pseudomean(time=LM[[time]]-tw$sl[j],event=LM[[status]],tmax=tw$w)
      LMdata<-rbind(LMdata,LM)
    }
  ...
  if (inter==TRUE){
    if(is.null(pre_cov)){
      ...
      LMdata_i<-add_interactions(LMdata1,pre_cov,func_covars=func_covars,func_lms =func_lms,sl=tw$sl)
      found_cols<-c()
      for (col in pre_cov) {
        matched_cols <- grep(paste0("^", col, "_"), colnames(LMdata_i$data), value = TRUE)
        found_cols <- c(found_cols, matched_cols)
  ...
  Model<-survival::coxph(f1,data=LMdata_i$data,method="breslow")
