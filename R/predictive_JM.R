#' Title Internal cross-validation function for joint model-based dynamic prediction models.
...
#' JM_pre<-predictive_JM(jointFit1,renal,method="CV",formulas=list(GFR~weight+age+ns(yearse,3),proteinuria~age+ns(yearse,3)),random_effect_formulas=list(~yearse|id,~yearse|id),surv_formula=Surv(time,status)~age+sex+weight,form_splines=form_splines,seq_len=seq(0,10,by=1),w=5,n=1,K=5,seed=1)
predictive_JM<-function(jointFit,data,method,formulas,random_effect_formulas,form_splines,surv_formula,seq_len,w,n,K,seed,r){
  ...
  if (method=="CV"){
    cindex_sm<-data.frame(array(NA,dim=c((5*n),length(seq_len))))
    BS_sm<-data.frame(array(NA,dim=c((5*n),length(seq_len))))
    AUC_sm<-data.frame(array(NA,dim=c((5*n),length(seq_len))))
    nrun<-0
    for(i in 1:(5*n)){
      tryCatch({
        set.seed(seed+10*i)
  ...
  if (method=="boot"){
    ...
    cindex_opt<-BS_opt<-AUC_opt<-data.frame(array(NA,dim=c((5*n),length(seq_len))))
    nrun<-0
    for (i in 1:(5*n)){
      tryCatch({
        set.seed(seed+10*i)
