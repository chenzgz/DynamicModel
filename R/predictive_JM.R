...
predictive_JM <- function(jointFit, data, method, formulas, random_effect_formulas,families =list(gaussian(),gaussian()),
                          ...
                          n_group = 10,dca_thresholds = seq(0.05, 0.50, by = 0.01)) {
  ...
  if (method == "split") {
    ...
    for (i in 1:(5 * n)) {
      tryCatch({
        ...
        for (k in seq_along(seq_len)) {
          ...
          dca_data_k <- all_pred[
            all_pred$time_points == time_point,
          ]

          if (nrow(dca_data_k) == 0) next

          dca_tmp <- cal_dca(
            pred_surv = dca_data_k$preds,
            data = dca_data_k,
            s = time_point,
            w = w,
            original_time = "Time",
    ...
    for (k in seq_along(seq_len)) {
      ...
      pred_app <- individual_JM_predict(
        jointFit,
        data,
        Tstart = time_point,
        Dt = w
    ...
    for (i in 1:(5 * n)) {
      tryCatch({
        ...
        for (k in seq_along(seq_len)) {
          ...
          pred_boot <- individual_JM_predict(
            jointFit1,
            train_data,
            Tstart = time_point,
            Dt = w
          ...
          pred_orig <- individual_JM_predict(
            jointFit1,
            data,
            Tstart = time_point,
            Dt = w
          ...
          if (!is.null(dca_boot) &&
            ...
            tmp$run <- i
            tmp$time_point <- time_point

            dca_opt[[length(dca_opt) + 1]] <-
              tmp
          }
